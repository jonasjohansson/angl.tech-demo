...
async function init() {
  ...
  // --- 3D voxel text system (Rubik's cube layout) ---
  // 3x3x3 cube: PC at center voxel, info surrounds it in 3D space
  // Voxels face-test against camera direction for visibility
  ...
  const voxelData = [
    // Front face (z = +1)
    { g: [0, 1, 1], title: 'ANGL', sub: null, size: 'hero' },
    { g: [0, -1, 1], title: 'REV—G', sub: null, size: 'hero' },
    // Right face (x = +1)
    { g: [1, 1, 0], title: '200', sub: 'MM RADIATOR', size: 'spec' },
    { g: [1, -1, 0], title: '5090', sub: 'GPU READY', size: 'spec' },
    // Left face (x = -1)
    { g: [-1, 1, 0], title: 'CNC', sub: 'ALUMINIUM', size: 'spec' },
    { g: [-1, -1, 0], title: 'SFX', sub: '1000W PSU', size: 'spec' },
    // Back face (z = -1)
    { g: [0, 1, -1], title: 'ITX', sub: 'FORM FACTOR', size: 'spec' },
    { g: [0, -1, -1], title: '2025', sub: 'STOCKHOLM', size: 'spec' },
    // Top center (y = +1)
    { g: [0, 1, 0], title: 'PC', sub: 'CASE', size: 'label' },
    // Accent corners
    { g: [1, 1, 1], title: 'DDR5', sub: null, size: 'label' },
    { g: [-1, -1, 1], title: 'M.2', sub: null, size: 'label' },
  ];

  const voxelInstances = [];
  const voxelVec = new THREE.Vector3();
  const voxelCamDir = new THREE.Vector3();
  const voxelOffsetDir = new THREE.Vector3();
  const cubeCenter = new THREE.Vector3(0, modelCenter.y, 0);

  voxelData.forEach((v, i) => {
    const el = document.createElement('div');
    el.className = `voxel-text ${v.size}`;
    el.style.opacity = '0';

    // Alignment: text radiates outward from center
    const tx = v.g[0] === -1 ? '-100%' : v.g[0] === 0 ? '-50%' : '0%';
    const ty = v.g[1] === 1 ? '-100%' : v.g[1] === 0 ? '-50%' : '0%';
    el.style.transform = `translate(${tx}, ${ty})`;
    el.style.textAlign = v.g[0] === -1 ? 'right' : v.g[0] === 0 ? 'center' : 'left';

    let html = `<span class="voxel-title">${v.title}</span>`;
    if (v.sub) html += `<span class="voxel-sub">${v.sub}</span>`;
    el.innerHTML = html;

    voxelContainer.appendChild(el);

    const worldPos = new THREE.Vector3(
      v.g[0] * VOXEL_SPACING,
      modelCenter.y + v.g[1] * VOXEL_SPACING,
      v.g[2] * VOXEL_SPACING
    );

    voxelInstances.push({
      data: v, el, worldPos, gridPos: v.g,
      revealDelay: 500 + i * 80,
    });
  });

  // Staggered reveal after loading
  function revealVoxels() {
    voxelInstances.forEach(({ el, revealDelay }) => {
      setTimeout(() => { el.dataset.revealed = 'true'; }, revealDelay);
    });
  }
  ...
  function updateVoxels() {
    ...
    voxelCamDir.subVectors(camera.position, cubeCenter).normalize();

    voxelInstances.forEach(({ worldPos, gridPos, el }) => {
      // Project world position to screen
      voxelVec.copy(worldPos);
      const projected = voxelVec.project(camera);
      const sx = projected.x * hw + hw;
      const sy = -projected.y * hh + hh;
      ...
      // Dot product: voxel offset vs camera direction
      voxelOffsetDir.set(gridPos[0], gridPos[1], gridPos[2]).normalize();
      const facing = voxelCamDir.dot(voxelOffsetDir);
      const revealed = el.dataset.revealed === 'true';
      const alpha = revealed ? Math.max(0, Math.min(0.9, (facing - 0.05) * 2.5)) : 0;
  ...
  function updateVoxelColors(bgColor) {
    if (!voxelContainer) return;
    const lum = bgColor.r * 0.299 + bgColor.g * 0.587 + bgColor.b * 0.114;
    voxelContainer.classList.toggle('dark-bg', lum < 0.45);
  }

  // --- Voxel grid wireframe visualization (Three.js overlay) ---
    ...
    for (const x of centers) {
      for (const y of centers) {
        for (const z of centers) {
          dp.push(x * S, y * S, z * S);
          if (x === 0 && y === 0 && z === 0) {
            ds.push(6.0); da.push(0.6);
          } else if (textOccupiedSet.has([x, y, z].join(','))) {
            ds.push(3.0); da.push(0.5);
          } else {
            ds.push(3.0); da.push(0.35);
          }
        }
      }
    }
  ...
  function updateVoxelViz() {
    ...
    voxelVizData.lineMat.opacity = 0.12 * voxelVizOpacity;
  ...
  // Staggered reveal of voxel text
  revealVoxels();
  ...
  function animate() {
    ...
    // Update dots and grid text color to match nearest context
    const activeCtxIdx = Math.round(scrollPosition);
    updateDots(activeCtxIdx);
    updateVoxelColors(contexts[activeCtxIdx].background);
    updateVoxelVizColors(contexts[activeCtxIdx].background);
