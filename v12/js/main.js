// ─── V12: Image-based product viewer — isometric camera ─────────
// No Three.js — pure DOM image swapping driven by scroll.
import { Pane } from 'tweakpane';

// ─── Frame grid ─────────────────────────────────────────────────
const STEP_DEG = 15;
const TOTAL_FRAMES = 24; // 0-23, 360° turntable
...
function init() {
  ...
  // ─── Wheel-driven continuous scroll ─────────────────────────────
  let currentFrameIndex = 0;
  let virtualScroll = 0;
  const SCROLL_PER_FRAME = 200; // pixels of wheel delta per frame change
  const TOTAL_SCROLL = SCROLL_PER_FRAME * TOTAL_FRAMES;
  ...
  // ─── Wheel handler (continuous loop) ──────────────────────────────
  let ticking = false;

  function updateFromVirtualScroll() {
    // Wrap virtualScroll to always stay in [0, TOTAL_SCROLL)
    virtualScroll = ((virtualScroll % TOTAL_SCROLL) + TOTAL_SCROLL) % TOTAL_SCROLL;
    const idx = Math.round(virtualScroll / SCROLL_PER_FRAME) % TOTAL_FRAMES;
    updateFrame(idx);
    updateHotspots();
  }
