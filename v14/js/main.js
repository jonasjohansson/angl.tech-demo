...
async function init() {
  ...
  // ─── Camera orbit — GUI-driven start/end interpolation ─────────
  // Start and end positions defined in the GUI. Scroll interpolates between them.
  const modelBox = new THREE.Box3().setFromObject(model);
  const modelCenter = modelBox.getCenter(new THREE.Vector3());
  const orbitTarget = new THREE.Vector3(0, modelCenter.y, 0);

  // Camera path: start → end (all GUI-editable)
  const cameraPath = {
    startAzimuth: 0,
    startElevation: 1.50,
    startDistance: 3.9,
  ...
  function updateCamera() {
    const t = currentFrameIndex / (TOTAL_FRAMES - 1); // 0→1
    const tSmooth = t * t * (3 - 2 * t); // ease-in-out

    const azimuth = THREE.MathUtils.degToRad(lerp(cameraPath.startAzimuth, cameraPath.endAzimuth, tSmooth));
    const el = lerp(cameraPath.startElevation, cameraPath.endElevation, tSmooth);
    const r = lerp(cameraPath.startDistance, cameraPath.endDistance, tSmooth);
    const zoom = lerp(cameraPath.startZoom, cameraPath.endZoom, tSmooth);
  ...
  // Helper: get current azimuth in degrees from camera path
  function getCurrentAzimuthDeg() {
    const t = currentFrameIndex / (TOTAL_FRAMES - 1);
    const tSmooth = t * t * (3 - 2 * t);
    return lerp(cameraPath.startAzimuth, cameraPath.endAzimuth, tSmooth);
  }

  function updateHotspots() {
    while (calloutSvg.firstChild) calloutSvg.removeChild(calloutSvg.firstChild);

    // Compute current azimuth from the path interpolation
    const currentDeg = ((getCurrentAzimuthDeg() % 360) + 360) % 360;
    const currentRad = THREE.MathUtils.degToRad(currentDeg);
    ...
    stopDotElements.forEach((dot, i) => {
      dot.classList.toggle('active', i === majorIndex);
    });
  ...
  const PARAMS = {
    // Camera path (start) — synced from cameraPath
    startAzimuth: 0,
    startElevation: 1.50,
    startDistance: 3.9,
    startZoom: 6.0,
  ...
  const fCamStart = pane.addFolder({ title: 'Camera Start' });
  fCamStart.addBinding(PARAMS, 'startAzimuth', { min: -720, max: 720, step: 1, label: 'azimuth °' }).on('change', (ev) => { cameraPath.startAzimuth = ev.value; });
  fCamStart.addBinding(PARAMS, 'startElevation', { min: 0, max: 1.5, step: 0.01, label: 'elevation' }).on('change', (ev) => { cameraPath.startElevation = ev.value; });
  fCamStart.addBinding(PARAMS, 'startDistance', { min: 1.5, max: 8, step: 0.1, label: 'distance' }).on('change', (ev) => { cameraPath.startDistance = ev.value; });
  fCamStart.addBinding(PARAMS, 'startZoom', { min: 1.5, max: 6, step: 0.1, label: 'zoom' }).on('change', (ev) => { cameraPath.startZoom = ev.value; });

  // Camera — End position
  const fCamEnd = pane.addFolder({ title: 'Camera End' });
  fCamEnd.addBinding(PARAMS, 'endAzimuth', { min: -720, max: 720, step: 1, label: 'azimuth °' }).on('change', (ev) => { cameraPath.endAzimuth = ev.value; });
  fCamEnd.addBinding(PARAMS, 'endElevation', { min: 0, max: 1.5, step: 0.01, label: 'elevation' }).on('change', (ev) => { cameraPath.endElevation = ev.value; });
  fCamEnd.addBinding(PARAMS, 'endDistance', { min: 1.5, max: 8, step: 0.1, label: 'distance' }).on('change', (ev) => { cameraPath.endDistance = ev.value; });
  fCamEnd.addBinding(PARAMS, 'endZoom', { min: 1.5, max: 6, step: 0.1, label: 'zoom' }).on('change', (ev) => { cameraPath.endZoom = ev.value; });
...
init().catch((err) => {
  console.error('V14 init failed:', err);
