...
// ─── Hotspot definitions ─────────────────────────────────────────
// 8 stops at 45° intervals. Each stop has 2-3 hotspots.
// anchorLocal: point on/near the model (in local model-space, y-up after rotation)
// panelSide: 'left' or 'right' — which side the text panel sits
const STOPS = [
  {
    angle: 0, // Front
    label: 'front',
    hotspots: [
      {
        anchorLocal: new THREE.Vector3(0, 0.35, 0.25),
        panelSide: 'right',
        tag: '01',
        title: 'Front Panel',
        desc: 'Precision-machined aluminium face with minimal design language.',
      },
      {
        anchorLocal: new THREE.Vector3(0, 0.05, 0.25),
        panelSide: 'left',
        tag: '02',
        title: 'Ventilation',
        desc: 'Optimized airflow channels for silent, efficient cooling.',
      },
    ],
  },
  {
    angle: Math.PI / 4, // Front-Right (45°)
    label: 'front-right',
    hotspots: [
      {
        anchorLocal: new THREE.Vector3(0.2, 0.3, 0.2),
        panelSide: 'right',
        tag: '03',
        title: 'Edge Profile',
        desc: 'Chamfered edges at precise 45° angles. CNC-finished surface.',
      },
      {
        anchorLocal: new THREE.Vector3(0.2, 0.05, 0.15),
        panelSide: 'right',
        tag: '04',
        title: 'Build Quality',
        desc: 'Unibody aluminium construction. No visible fasteners.',
      },
    ],
  },
  {
    angle: Math.PI / 2, // Right (90°)
    label: 'right',
    hotspots: [
      {
        anchorLocal: new THREE.Vector3(0.28, 0.25, 0),
        panelSide: 'right',
        tag: '05',
        title: 'Side Panel',
        desc: 'Removable side panel for easy component access.',
      },
      {
        anchorLocal: new THREE.Vector3(0.28, 0.05, -0.1),
        panelSide: 'right',
        tag: '06',
        title: 'Expansion',
        desc: 'Full-length GPU support with dedicated airflow path.',
      },
    ],
  },
  {
    angle: (3 * Math.PI) / 4, // Rear-Right (135°)
    label: 'rear-right',
    hotspots: [
      {
        anchorLocal: new THREE.Vector3(0.15, 0.35, -0.2),
        panelSide: 'right',
        tag: '07',
        title: 'Exhaust',
        desc: 'Rear exhaust system with integrated cable routing.',
      },
    ],
  },
  {
    angle: Math.PI, // Rear (180°)
    label: 'rear',
    hotspots: [
      {
        anchorLocal: new THREE.Vector3(0, 0.3, -0.28),
        panelSide: 'left',
        tag: '08',
        title: 'I/O Panel',
        desc: 'Full connectivity — USB-C, USB-A, DisplayPort, HDMI, Ethernet.',
      },
      {
        anchorLocal: new THREE.Vector3(0, 0.05, -0.28),
        panelSide: 'right',
        tag: '09',
        title: 'Power Supply',
        desc: 'Integrated SFX PSU with modular cabling.',
      },
    ],
  },
  {
    angle: (5 * Math.PI) / 4, // Rear-Left (225°)
    label: 'rear-left',
    hotspots: [
      {
        anchorLocal: new THREE.Vector3(-0.15, 0.25, -0.2),
        panelSide: 'left',
        tag: '10',
        title: 'Cable Management',
        desc: 'Internal channels route cables cleanly behind the motherboard tray.',
      },
    ],
  },
  {
    angle: (3 * Math.PI) / 2, // Left (270°)
    label: 'left',
    hotspots: [
      {
        anchorLocal: new THREE.Vector3(-0.28, 0.3, 0),
        panelSide: 'left',
        tag: '11',
        title: 'Left Panel',
        desc: 'Solid aluminium side. Clean, unbroken surface.',
      },
      {
        anchorLocal: new THREE.Vector3(-0.28, 0.08, 0.1),
        panelSide: 'left',
        tag: '12',
        title: 'Acoustic Dampening',
        desc: 'Layered isolation pads minimize vibration and noise.',
      },
    ],
  },
  {
    angle: (7 * Math.PI) / 4, // Front-Left (315°)
    label: 'front-left',
    hotspots: [
      {
        anchorLocal: new THREE.Vector3(-0.15, 0.35, 0.2),
        ...
        desc: 'Scandinavian-inspired minimalism meets high-performance computing.',
      },
      {
        anchorLocal: new THREE.Vector3(-0.1, 0.05, 0.2),
        panelSide: 'left',
        tag: '14',
        title: 'Footprint',
        desc: 'Compact ITX form factor — fits any desk, powers any workflow.',
      },
    ],
  },
];
...
// ─── Frame grid ─────────────────────────────────────────────────
// 24 discrete frames at 15° intervals. Major stops every 3rd frame.
const STEP_DEG = 15;
const TOTAL_FRAMES = 360 / STEP_DEG + 1; // 25 — last frame returns to front
const FRAMES = Array.from({ length: TOTAL_FRAMES }, (_, i) => {
  const deg = (i * STEP_DEG) % 360;
  const rad = THREE.MathUtils.degToRad(deg);
  // Every 3rd frame is a major stop (0, 3, 6, ... and 24 wraps to front = stop 0)
...
async function init() {
  ...
  let frustumSize = 3.2; // zoomed out
  ...
  const orbitRadiusRef = { value: 4.0 }; // zoomed out for isometric
  const orbitElevationRef = { value: 0.55 }; // isometric-style elevated view
  ...
  // ─── Update hotspots + callout lines ───────────────────────────
  const MARGIN_LEFT = 60;   // left text column
  const MARGIN_RIGHT = 60;  // right text column from edge
  ...
  function updateHotspots() {
    ...
    // Update scroll indicator — highlight nearest major stop
    ...
    if (!isMajorStop) {
      // Hide all hotspots on intermediate frames
    ...
    hotspotElements.forEach(({ panel, hotspot, stopAngle }) => {
      ...
      // Project anchor point to screen
      const screen = project3DToScreen(hotspot.anchorLocal);

      // Text goes to far left or right margin
      ...
      // Draw horizontal callout line: anchor → elbow → panel
      // First a horizontal line from anchor to side, then if needed a short vertical
      const elbowX = lineEndX;
      const elbowY = panelY + 14;

      // Line from anchor dot to elbow
      ...
      // Short vertical segment from elbow to text
  ...
  fCam.addBinding(PARAMS, 'elevation', { min: 0, max: 1.2, step: 0.01, label: 'elevation' }).on('change', (ev) => {
    // Update the mutable reference — need to reassign via a wrapper
  ...
  fLight.addBinding(PARAMS, 'ambientColor', { label: 'ambient color' }).on('change', (ev) => {
    ambient.color.set(ev.value);
  });
  fLight.addBinding(PARAMS, 'keyIntensity', { min: 0, max: 8, step: 0.1, label: 'key' }).on('change', (ev) => {
    keyLight.intensity = ev.value;
  });
  fLight.addBinding(PARAMS, 'keyColor', { label: 'key color' }).on('change', (ev) => {
    keyLight.color.set(ev.value);
  });
  fLight.addBinding(PARAMS, 'fillIntensity', { min: 0, max: 3, step: 0.1, label: 'fill' }).on('change', (ev) => {
    fillLight.intensity = ev.value;
  });
  fLight.addBinding(PARAMS, 'fillColor', { label: 'fill color' }).on('change', (ev) => {
    fillLight.color.set(ev.value);
  });
  fLight.addBinding(PARAMS, 'rimIntensity', { min: 0, max: 4, step: 0.1, label: 'rim' }).on('change', (ev) => {
    rimLight.intensity = ev.value;
  });
  fLight.addBinding(PARAMS, 'rimColor', { label: 'rim color' }).on('change', (ev) => {
    rimLight.color.set(ev.value);
  });
  ...
  fMat.addBinding(PARAMS, 'metalness', { min: 0, max: 1, step: 0.01 }).on('change', (ev) => {
    forEachMaterial((m) => { m.metalness = ev.value; m.needsUpdate = true; });
  });
  fMat.addBinding(PARAMS, 'roughness', { min: 0, max: 1, step: 0.01 }).on('change', (ev) => {
    forEachMaterial((m) => { m.roughness = ev.value; m.needsUpdate = true; });
  });
  fMat.addBinding(PARAMS, 'envMapIntensity', { min: 0, max: 5, step: 0.1, label: 'env map' }).on('change', (ev) => {
    forEachMaterial((m) => { m.envMapIntensity = ev.value; m.needsUpdate = true; });
  });

  // Post-FX folder
  const fFx = pane.addFolder({ title: 'Post-FX' });
  fFx.addBinding(PARAMS, 'exposure', { min: 0.5, max: 4, step: 0.05 }).on('change', (ev) => {
    renderer.toneMappingExposure = ev.value;
  });
  fFx.addBinding(PARAMS, 'grainAmount', { min: 0, max: 0.06, step: 0.001, label: 'grain' }).on('change', (ev) => {
    filmPass.uniforms.uGrainAmount.value = ev.value;
  });

  // Ground folder
  const fGround = pane.addFolder({ title: 'Ground' });
  fGround.addBinding(PARAMS, 'shadowOpacity', { min: 0, max: 1, step: 0.05, label: 'shadow opacity' }).on('change', (ev) => {
    groundPlane.material.opacity = ev.value;
  });
...
init().catch((err) => {
  console.error('V6 init failed:', err);
